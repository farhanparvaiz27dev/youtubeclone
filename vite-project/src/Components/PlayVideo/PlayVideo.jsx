...
// const PlayVideo = ({ videoId }) => {
  ...
  const fetchOtherData = async () => {
    ...
    const Comments_url = ` https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&videoId=${videoId}&maxResults=45&key=${API_KEY}  `;
  ...
  return (
    <div className='paly-video'>
      ...
      <div className="paly-video-info">
        <p>{apiData ? value_converter(apiData.statistics.viewCount) : "12k"} Views &bull;
          {apiData?.snippet ? moment(apiData.snippet.publishedAt).fromNow() : ""}
        </p>
        <div>
          <span><img src={like} alt="" />{value_converter(apiData ? apiData.statistics.likeCount : " 123")}</span>
        ...
        {Array.isArray(commentData) && commentData.map((item, index) => {
          return (
            <div key={index} className="comment">
              <img
                src={item?.snippet?.topLevelComment?.snippet?.authorProfileImageUrl || null}
